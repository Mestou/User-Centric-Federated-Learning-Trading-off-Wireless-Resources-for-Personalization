...
for n, color, marker in zip(algorithm_names, colors, markers):
    ...
    if (n!="PER" and n!="FedFomo"):
        ...
        plt.plot((1+ul_dl_ratio)*np.arange(0, 250), data, color = "goldenrod",label=n, marker=marker,markersize=4,markevery=5)
    elif (n == "PER"):
        ...
        plt.plot((1+ul_dl_ratio*35)*np.arange(0,250),loss,label=n,color=[0.93,0.23,0.51], marker='o',markersize=4,markevery=5)
        i=i+1
    else:
        loss = np.mean(np.load( f'.\Algorithm = {n} - dataset = Sentiment - clients = 35 - clustering = off (relevant for PER only) - run = 0.npy', allow_pickle=True),axis=1)
        loss = np.mean(loss,axis=0)
