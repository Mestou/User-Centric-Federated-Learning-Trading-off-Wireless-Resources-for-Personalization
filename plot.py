...
colors = ['blue', 'green', 'violet', "gray", 'mediumpurple', 'goldenrod','purple']
markers = ['o', '>', 's', 'o', 's', '>','o']
nodes = 35
ul_dl_ratio = 4
epsilon = 0.3
...
for n, color, marker in zip(algorithm_names, colors, markers):
    ...
    if (n== "PER"):
        ...
        # # Plot the performance of the current algorithm
        plt.plot((1*nodes + nodes+ 1)*x/100, smoothed_y[:150], color=[1,0.08,0.58], marker=marker, linestyle='--', linewidth=3, markersize=7,
                 label=f'{n} w/ 35 models', markevery=4)

        n = "PER"
        data = np.load(
            f'.\Results\Algorithm = {n} - dataset = Sentiment - clients = 35 - clustering = on (relevant for PER only) - run = 0.npy')
        i = 16
        n = "Proposed w/ 16 models"
        data = np.mean(np.mean(data, axis=0), axis=0)
        # # Generate x-axis values based on the data length
        x = np.arange(0, 150)
        smoothed_y = savgol_filter(data, window_size)
        # # Plot the performance of the current algorithm
        plt.plot((1*16 + 16)*x/100, smoothed_y[:150], color="lime", marker=">", linestyle='--',  linewidth=3, markersize=7,
                 label=f"{n}", markevery=4)
        ...
        n = "FedFOMO"
        smoothed_y = savgol_filter(data[0], window_size)
        plt.plot((nodes*nodes + ul_dl_ratio*nodes + 1)*np.arange(0, 150)/100, smoothed_y[:150], color="goldenrod", marker=marker, linestyle='-', linewidth=3, markersize=7,
                     label=n, markevery=1)
        i = 2
        loss = []
        n = "FedFomo"
        ...
        x = np.arange(0, 150)
        smoothed_y = savgol_filter(data[0], window_size)
        plt.plot((i * nodes +  nodes ) * np.arange(0, 150)/100, smoothed_y[:150], color= "orange", marker=marker, linestyle='-',  linewidth=3, markersize=7,
                 label=f"{n} w/ M = {i}", markevery=4)
    # elif n == "LOC":
    #     data = np.load(
    #         f'.\Results\Algorithm = {n} - dataset = Sentiment - clients = 35 - clustering = off (relevant for PER only) - run = 0.npy')
    #     data = np.mean(np.mean(data, axis=0), axis=0)
    ...
    #              linewidth=2, markersize=4, label=n, markevery=4)
        ...
        smoothed_y = savgol_filter(data, window_size)
        # # Plot the performance of the current algorithm
        plt.plot((1 + nodes)*x/100, smoothed_y[:150], color=color, marker=marker, linestyle='--', linewidth=3, markersize=7, label=n,markevery = 4)
...
plt.xlabel(r'Communication Cost [MB]',fontsize=22)
plt.xlim([1,5000/100])
...
for n, color, marker in zip(algorithm_names, colors, markers):
    ...
    if (n== "PER"):
        ...
        smoothed_y = savgol_filter(data[0], window_size)
        # # Plot the performance of the current algorithm
        plt.plot(np.arange(0, 300), smoothed_y[:300], color=[1,0.08,0.58], marker=marker, linestyle='--', linewidth=3, markersize=7,
                 label=f'{n} w/ 35 models', markevery=4)
        ...
        smoothed_y = savgol_filter(data, window_size)
        # # Plot the performance of the current algorithm
        plt.plot(np.arange(0, 150), smoothed_y[:150], color="lime", marker=">", linestyle='-',  linewidth=3, markersize=7,
                 label=f"{n}", markevery=4)
        ...
        n="FedFOMO"
        smoothed_y = savgol_filter(data[0], window_size)
        plt.plot(np.arange(0, 300), smoothed_y[:300], color="goldenrod", marker=marker, linestyle='-', linewidth=3, markersize=7,
                     label=n, markevery=4)
        i = 2
        loss = []
        n="FedFomo"
        ...
        n = "FedFOMO"
        smoothed_y = savgol_filter(data[0], window_size)
        plt.plot(np.arange(0, 150), smoothed_y[:150], color= "orange", marker=marker, linestyle='-',  linewidth=3, markersize=7,
                 label=f"{n} w/ M = {i}", markevery=4)
    # elif n == "LOC":
    #     data = np.load(
    #         f'.\Results\Algorithm = {n} - dataset = Sentiment - clients = 35 - clustering = off (relevant for PER only) - run = 0.npy')
    ...
    #              linewidth=2, markersize=4, label=n, markevery=4)
        ...
        smoothed_y = savgol_filter(data, window_size)
        # # Plot the performance of the current algorithm
        plt.plot(np.arange(0, 150), smoothed_y[:150], color=color, marker=marker, linestyle='--',  linewidth=3, markersize=7, label=n,markevery = 4)
